...
#
#     Version 12.0, 16.2.2012 Simone Cesca
#  
...
# 
#	28 Nov 2012: changed by Susana Custodio for Mac OS.
#		- change from 'r' to 'r+' all read files, this avoids the problem with flush()
#		- add gmtset LABEL_FONT_SIZE 10p ANNOT_FONT_SIZE_SECONDARY 10p 
#			ANNOT_FONT_SIZE_PRIMARY 10p to gmt scripts
#		- convert outputs from ps to pdf, starting with "a_"
...
#      scmom=calcScMoment(float(omts[0]),float(omts[1]),float(omts[2]),float(omts[3]),float(omts[4]),float(omts[5]))
      ...
      if inv_param['MT_CLVD'].upper()=="TRUE":
         clstm11=float(smom)/math.sqrt(3)
         clstm22,clstm33=-(float(smom)/math.sqrt(3))/2,-(float(smom)/math.sqrt(3))/2
         clstm12,clstm13,clstm23=0,0,0
...
#   f.write("set_source_location "+inv_param['LATITUDE_NORTH']+" "+inv_param['LONGITUDE_EAST']+" 0\n")
   ...
   for startsol in start_mt_solutions:   
      ...
      f.write(line)
      if (inv_step == '1'):
         f.write(line_mask(stype,['m11','m22','m33','m12','m13','m23']))
         f.write("minimize_lm\n")
         f.write("get_source_subparams\n")	    
         f.write("get_global_misfit\n")
         i=i+1
...
def plotDuration(inv_step,inv_param):
   ...
   fappdurpdf=os.path.join(inv_param['INVERSION_DIR'],'aaa_apparentdurations.pdf')
   ...
   fgmt.write("#!/bin/bash\n")
   fgmt.write("gmtset LABEL_FONT_SIZE 10p ANNOT_FONT_SIZE_SECONDARY 10p ANNOT_FONT_SIZE_PRIMARY 10p \n")
...
#   fgmt.write("psbasemap -X1.5 -Y14 -R -JM10 -K -O -V -P -BSneWa5f5:' ':/a5f5:' ': >>"+fappdurps+"\n")
   ...
   fgmt.write("ps2pdf "+fappdurps+ " "+ "fappdurpdf \n")
   fgmt.flush()
   fgmt.close()
...
def plotMTSolution(inv_step,inv_param,bestdc,mt_solutions,best_mt_solutions,traces):
   ...
   fplotpdf=os.path.join(dinv,'aaa_step'+inv_step+'.mtsolution.pdf')
...
#Create file earthquakeinfo.dat
   ...
   f=open(fgmtplot,'w')
   f.write("#!/bin/bash\n")
   f.write("gmtset LABEL_FONT_SIZE 10p ANNOT_FONT_SIZE_SECONDARY 10p ANNOT_FONT_SIZE_PRIMARY 10p \n")
   line="pstext <"+feqinfo+" -X2 -Y20 -JX10/8 -R0/14/0/28 -K -P >"+fplot+"\n"
   f.write(line)
#  
#  Fit header 3 Fit MT matrix
   line="psxy <<he -X2.2 -Y2.6 -JX6.1/3 -R0/6.1/0/3 -Bwsen -G0/0/220 -W0/0/220 -K -O -V -P >> "+fplot+"\n"
   f.write(line)
   f.write("he\n")
   f.write("pstext <<fits -X0 -Y0 -JX6.1/3 -R0/6/0/3 -K -O >>"+fplot+"\n")
...
#  Plot matrix 
   if (inv_step == '1'):        
      f.write("pstext <<fits -X-2.2 -Y-22 -JX17/20 -R0/20/0/20 -K -O >>"+fplot+"\n")
      ...
      f.write("pstext <<fits -X0 -Y14.5 -JX17/2 -R0/17/0/3 -K -O >>"+fplot+"\n")
...
#         x1,x2=inv_param['START_PLOT_STEP'+inv_step],str(float(inv_param['LEN_PLOT_STEP'+inv_step])-float(inv_param['START_PLOT_STEP'+inv_step]))
      ...
      for comp in usedcomp:
          ...
          f.write("0 0\n0 0\nxscale\n")
   if (inv_step == '2'):  
      f.write("pstext <<fits -X-2.2 -Y-22 -JX20/20 -R0/20/0/20 -K -O >>"+fplot+"\n")
      ...
      f.write("fits\n")
#  Fit ISO matrix 
      line="psxy <<he -X2 -Y4 -JX6.1/3 -R0/6.1/0/3 -Bwsen -G0/0/220 -W0/0/220 -K -O -V -P >> "+fplot+"\n"
      f.write(line)
      f.write("he\n")
      f.write("pstext <<fits -X0 -Y0 -JX6.1/3 -R0/6/0/3 -K -O >>"+fplot+"\n")
      ...
      f.write("fits\n")   
#   Fit CLVD matrix
      line="psxy <<he -X0 -Y5 -JX6.1/3 -R0/6.1/0/3 -Bwsen -G0/0/220 -W0/0/220 -K -O -V -P >> "+fplot+"\n"
      f.write(line)
      f.write("he\n")
      f.write("pstext <<fits -X0 -Y0 -JX6.1/3 -R0/6.1/0/3 -K -O >>"+fplot+"\n")
...
#   Fit DC matrix
      ...
      f.write(line)
      f.write("he\n")
      f.write("pstext <<fits -X0 -Y0 -JX6/3 -R0/6/0/3 -K -O >>"+fplot+"\n")
      ...
      f.write("fits\n")       
#   Plot percentage
      f.write("pstext <<fits -X12 -Y-13 -JX5/21 -R0/5/0/21 -K -O >>"+fplot+"\n") 
...
#  Plot BeachBalls
   if (inv_step == '1'): 
      f.write("psxy < "+TableMT_lines+" -X0 -Y19.5 -JX3/3 -R-1/1/-1/1 -W2/0 -P -K -O -M >> "+fplot+"\n")
      f.write("pstext <<endplot -O -X0 -Y0 -JX -R -P >>"+fplot+"\n")
      f.write("endplot\n")
   if (inv_step == '2'): 
      f.write("makecpt -Cpolar -Z > "+cpt+"\n")
      f.write("psxy < "+TableMT_fill+" -X-4 -Y21 -JX3/3 -R-1/1/-1/1 -W2/0 -P -C"+cpt+" -K -O -M -L >> "+fplot+"\n")
      f.write("psxy < "+TableDC_fill+" -X0 -Y-8 -JX3/3 -R-1/1/-1/1 -W2/0 -P -C"+cpt+" -K -O -M -L >> "+fplot+"\n")
      f.write("psxy < "+TableCLVD_fill+" -X0 -Y-5 -JX3/3 -R-1/1/-1/1 -W2/0 -P -C"+cpt+" -L -K -O -M>> "+fplot+"\n")
      f.write("psxy < "+TableISO_fill+" -X0 -Y-5 -JX3/3 -R-1/1/-1/1 -W2/0 -P -C"+cpt+" -K -O -M>> "+fplot+"\n")
      f.write("psxy <"+TableISO_lines+" -X0 -Y0 -JX3/3 -R-1/1/-1/1 -W2/0 -P -K -O -M>> "+fplot+"\n")
      f.write("pstext <<endplot -O -X0 -Y0 -JX -R -P >>"+fplot+"\n")
      f.write("ps2pdf "+fplot+ " " +fplotpdf+"\n")
      f.write("endplot\n")
   f.flush()
   f.close()
...
def plotDCSolution(inv_step,inv_param,all,best,traces):
   ...
   fplotpdf=os.path.join(dinv,'aaa_step'+inv_step+'.ptsolution.pdf')
   if (inv_step == '1'):
...
def plotEikSolution(inv_step,inv_param,all,best,traces):
   ...
   fplotpdf=os.path.join(dinv,'aaa_step'+inv_step+'.eiksolution.pdf')
   if (inv_step == '3'):
